  ...
  useEffect(() => {
    ...
    switch (selectedRange) {
      ...
      case 'current':
        ...
        break;
      default:
        setStart(startOfMonth(addMonths(now, -1)));
        setEnd(endOfMonth(addMonths(now, -1)));
        break;
    }
  }, [selectedRange]);
  ...
  return (
    <>
      <Box display={'flex'} flexDirection="column" gap={2}>
        ...
        {selectedRange === 'week' && <WeekDetailView data={data} holidays={allHolidays} />}
...
function UserTimesGrid({ dailyEntries, handleDialogRequest }: UserTimesGridProps) {
  ...
  const renderHours = (value: string | number | undefined) => {
    if (!value) {
      return <Chip size="small" label={'✓'} color={'success'} />;
    }
